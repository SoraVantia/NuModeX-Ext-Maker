...
1. 이용약관에 동의 (첫 실행 시).
2. 드롭다운 메뉴에서 모델을 선택. 온디바이스 모델은 API 키가 필요 없지만, 채팅과 편집만 가능하며 빌드는 할 수 없습니다.
3. 설정에서 클라우드 AI 제공업체의 API 키를 입력.
4. 빌드하려는 내용을 설명한 후 "확장 프로그램 빌드" 또는 "웹사이트 빌드"를 클릭.
5. 생성된 파일을 ZIP으로 다운로드하고 브라우저에 로드.

자세한 설정 방법, 온디바이스 AI 구성, 문제 해결 및 팁은 [시작 가이드](getting-started-ko-3-26-2026.md)를 참조하세요.

## API 키

클라우드 AI 제공업체를 사용하려면 자체 API 키가 필요합니다. 온디바이스 모델은 API 키가 필요 없지만, 채팅과 편집만 가능하며 빌드는 할 수 없습니다. 선택한 제공업체에서 받으세요. API 키는 브라우저에 로컬로 저장되며 SoraVantia GK나 제3자에게 전송되지 않습니다.
