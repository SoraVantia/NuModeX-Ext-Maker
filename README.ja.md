...
- ZIPからインポート。既存のブラウザ拡張機能やウェブサイトをZIPファイルから読み込み、AIで表示・編集
...
1. 利用規約に同意（初回起動時）。
2. ドロップダウンメニューからモデルを選択。オンデバイスモデルはAPIキーが不要ですが、チャットと編集のみに対応し、構築はできません。
3. 設定でクラウドAIプロバイダーのAPIキーを入力。
4. 構築したいものを説明して「拡張機能を構築」または「ウェブサイトを構築」をクリック。
5. 生成されたファイルをZIPでダウンロードし、ブラウザにロード。

詳細なセットアップ手順、オンデバイスAIの設定、トラブルシューティング、ヒントについては[スタートガイド](getting-started-ja-3-26-2026.md)をご覧ください。

## APIキー

クラウドAIプロバイダーを使用するには自分のAPIキーが必要です。オンデバイスモデルはAPIキーが不要ですが、チャットと編集のみに対応し、構築はできません。選択したプロバイダーから取得してください。APIキーはブラウザにローカル保存され、SoraVantia合同会社や第三者に送信されることはありません。
