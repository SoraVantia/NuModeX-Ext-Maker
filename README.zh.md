...
- 内置代码查看器和文件树
- 一键 ZIP 下载生成的扩展程序
- 从ZIP导入。从ZIP文件加载现有的浏览器扩展程序或网站，使用AI查看和编辑
...
![Extension Builder](assets/extension-builder/numodex-ext-maker-extension-builder-zh.png)
...
1. 接受服务条款（首次启动时）。
2. 从下拉菜单中选择模型。设备端模型无需 API 密钥，但仅可用于聊天和编辑，无法构建。
3. 在设置中输入您的云端 AI 提供商的 API 密钥。
4. 描述您想要构建的内容，然后点击"构建扩展程序"或"构建网站"。
5. 将生成的文件下载为 ZIP 并加载到浏览器中。

有关详细的设置说明、设备端 AI 配置、故障排除和提示，请参阅[入门指南](getting-started-zh-3-26-2026.md)。

## API 密钥

使用云端 AI 提供商需要您自己的 API 密钥。设备端模型无需 API 密钥，但仅可用于聊天和编辑，无法构建。从您选择的提供商获取。API 密钥本地存储在您的浏览器中，永远不会发送给 SoraVantia GK 或任何第三方。
